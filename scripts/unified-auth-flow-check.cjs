...
(async () => {
  ...
  page.handleWechatPhone({ detail: { code: "phone-code" } }); await settle(); assert.equal(wechatPhoneCalls, 1); assert.equal(page.data.stage, "profile"); assert.equal(redirected, "");
