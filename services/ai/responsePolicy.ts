...
const chooseMove = (signals: ConversationSignals) => {
  ...
  if (signals.isVagueOrStuck) {
    return "允许模糊：先允许说不清，再给一个很小的表达入口；不要问抽象大问题，也不要替用户判断状态，不要命令用户“挑一个说”。如果用户说别追问，就不要再问问题，可以停在用户这句话上；不要让用户回标点或表情。";
  }
...
export const buildResponsePolicyGuidance = ({
  ...
  return [
    ...
    "- 对话已经重复围绕累/烦/难受时，不要用“我在这儿/陪着你/陪你”作为收尾；这会变成空泛安抚。只贴住用户刚说的具体词。",
    "- 首轮低能量回复不能把对话关掉；不要说“先安静待会儿也行/想歇会儿吗/不说话也行/待着就好”，要留下一个表达上的小入口，而不是行动建议。",
    "- 如果用户没有给出新信息，不要重复上一轮同一种安慰话术；换成更具体、更容易回答的入口。",
    "- 用户模糊、短答或卡住时，不要问“想说什么/你想说什么都行/等你想说的时候再说/直接说你想说的/为什么/发生了什么”这类大问题；给一个低压力选项或一个词入口。即使用户明确说别追问/别让我想，也不要用“回个句号/发个表情/让我知道你在”这种输入任务式入口。",
