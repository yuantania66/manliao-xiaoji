...
const MICRO_ENTRY_TERMS = ["身体累", "心里累", "都累", "一个字", "说个字", "一个词", "说个词", "句号", "点一下", "表情", "选一个", "哪一", "最重", "最明显"];
...
const CLOSED_CONVERSATION_PATTERNS = [
  /安静待(一)?会儿/,
  /先待(一)?会儿/,
  /想歇(一)?会儿/,
  /不说话也行/,
...
const runLocalJudge = ({
  ...
  if (
    knownUserStateTerms.includes("累") &&
  ...
  if (
    knownUserStateTerms.includes("累") &&
    CLOSED_CONVERSATION_PATTERNS.some((pattern) => pattern.test(assistantReply)) &&
    !MICRO_ENTRY_TERMS.some((term) => assistantReply.includes(term))
  ) {
  ...
  if (
    knownUserStateTerms.includes("累") &&
    /身体累|心里累/.test(assistantReply) &&
    /不知道|嗯|算了|还是|也不是|已经说了|别再/.test(userMessage)
  ) {
    issues.add("reasks_known_information");
    issues.add("lack_of_empathy");
  }
