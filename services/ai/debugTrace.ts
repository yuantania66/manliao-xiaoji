    ...
    },
    {
      ...
      evidence: controlTrace
        ? [
            ...
            `ordinaryPosture=${controlTrace.responsePlan.ordinaryPosture?.mode ?? "priority_owned"}`,
            `ordinaryPostureSpans=${controlTrace.responsePlan.ordinaryPosture?.sourceSpans.map((span) => `${span.source}:${span.sourceTurnId}:${span.start}-${span.end}:${JSON.stringify(span.text)}`).join(" | ") ?? "none"}`,
            `ordinaryPostureTargets=${controlTrace.responsePlan.ordinaryPosture?.requiredContribution.targetSpanIndexes.join(",") ?? "none"}`,
            `ordinaryPostureContribution=${controlTrace.responsePlan.ordinaryPosture?.requiredContribution.instruction ?? "none"}`,
            `purposeSubjectOwnership=${controlTrace.interpretation.purposeSubjectOwnershipAuthority?.ownership ?? controlTrace.interpretation.purposeSubjectOwnershipAuthority?.reason ?? "not_evaluated"}`,
            `purposeSubjectOwnershipUsed=${controlTrace.interpretation.purposeSubjectOwnershipAuthority?.used ?? false}`,
            `purposeSubjectOwnershipContract=${controlTrace.interpretation.purposeSubjectOwnershipAuthority?.contractSha256 ?? "none"}`,
