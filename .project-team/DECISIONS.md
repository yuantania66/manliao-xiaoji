...
| 2026-08-08 | 封存 PHM-A Reciprocal/Unclear Candidate Reconciliation | 真实 committed trace 复现 `[reciprocates_move, unclear]`；精确同 target 模型关系现在只淘汰合并器自产的 adjacency fallback，专项、相邻门、独立 Reviewer 与完整 launch 均通过 | Planner 对真实 `unclear` 的 defer 合同不变；错 target、缺 target、低置信、provider failure 与模型真歧义继续 fail closed |
