...
| `check:unified-auth-flow` | 小程序统一登录状态机：微信、微信绑定手机号码、游客三入口；游客可从“我的”或“设置”重新进入登录选择且返回时保留游客身份；短信分支保留但当前 UI 不可达；注册完成后可跳过的资料建议与游客身份编辑 |
| `check:production-env-audit` | 当前短信延后时允许短信配置全缺失；一旦出现部分短信配置仍拒绝发布，完整配置保持可验证 |
...
微信绑定手机号码登录门使用 `WECHAT_PHONE_LOGIN_TEST_DATABASE_URL` 指向明确命名的隔离 test/ci 数据库。它必须通过真实路由验证新账号创建、既有手机号/微信账号绑定、同一账号重复登录、冲突拒绝和并发同号隔离；Mock 只能替代微信网络响应，不能替代路由、事务和数据库断言。
...
- 微信开发者工具编译、预览，无阻断错误；
- 登录选择页按设备当前时区显示对应的清晨、白天、黄昏或月夜背景；前后台跨时段后重新显示时应刷新，登录弹层与操作位置不得变化；
- 体验版 HTTPS 合法域名、真实 `code2Session` 微信登录、`getPhoneNumber` 微信绑定号码登录，并保留游客入口；当前候选不得展示或进入其他手机号短信验证码登录；
- 微信或微信绑定手机号码验证成功后必须立即形成持久注册账号；随后可展示一次能跳过的头像昵称建议，缺少昵称或头像不得阻塞使用；游客不计入注册账号；
- 资料页不得自动读取微信头像或昵称；只有用户主动选择微信头像、相册、相机或昵称建议/手动输入后才处理本次结果。头像不要求真人照片，并核对隐私政策明确头像不进入 AI、不做人脸识别；
- 游客入口必须先显示本机存储提醒，再由用户确认本机昵称与头像；返回或取消不得自动进入游客；
...
在获授权的生产等价环境验证真实微信、微信手机号、对象存储/CDN、数据库 migration、域名和模型 provider。当前候选的短信登录延后，不要求短信凭据；恢复短信入口前必须配置完整短信参数并通过审计，再完成真实登录与注销短信验证。`smoke:prod` 只证明其覆盖的接口存活，不替代完整人工主链路，也不得接触未经授权的生产明文或真用户数据。
