...
| `check:profile-completion-gate-e2e` | 新账号资料完成前业务阻塞、完成后放行与放弃登录清理 |
...
| `check:unified-auth-flow` | 小程序统一登录状态机：微信、微信手机号、游客三入口；短信分支保留但当前 UI 不可达；资料必填门与游客身份 |
...
- 微信或手机号登录后，缺少昵称或头像时必须停留在统一登录流程的资料必填页，不得先展示“我的”或普通登录态功能；头像和昵称齐全后才能进入，允许放弃登录但不可跳过；已有完整资料再次编辑时可以取消；
