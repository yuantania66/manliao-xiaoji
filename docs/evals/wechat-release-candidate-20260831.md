...
**短信延后候选已于 2026-09-01 部署生产；微信审核尚未提交。**

当前短信延后候选的 21 个数据库迁移、完整发布门和生产构建均已通过。公开登录范围已收窄为微信登录、微信绑定手机号码登录和游客模式，短信入口暂缓。生产现运行封存提交 `dc1d010`、21 个迁移和 Build ID `VMncqpVGHM4vSPt86SVn7`；基础 Smoke 3/3、真实 Qwen 合成游客欢迎语和“你好”聊天均通过。GitHub 当时无法连接，因此生产采用 SHA-256 校验后的 tar 包直接蓝绿部署，远端主分支同步仍待网络恢复后完成。
...
- 当前生产应用 release：`4c0b72e`；Nginx 代理到新版 `3102`，`dc1d010` 在 `3101`、旧版 `5625262` 在 `3100` 保留作即时回滚。
...
3. 部署后运行基础 Smoke 与微信/微信绑定手机号码登录、资料完成、头像、小记、聊天、观察、注销、文件清理和跨账号隔离的合成主链路 Smoke。
...
7. Composer 三自然日门不再作为发布阻断；后续仅在北京时间 17:00 固定窗口采样并报告趋势，不跨天重包装为新的三日证据。
