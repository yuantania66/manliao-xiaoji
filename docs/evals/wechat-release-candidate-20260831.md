...
当前短信延后候选的 21 个数据库迁移、完整发布门和生产构建均已通过。公开登录范围已收窄为微信登录、微信手机号登录和游客模式，短信入口暂缓。生产现运行封存提交 `dc1d010`、21 个迁移和 Build ID `VMncqpVGHM4vSPt86SVn7`；基础 Smoke 3/3、真实 Qwen 合成游客欢迎语和“你好”聊天均通过。GitHub 当时无法连接，因此生产采用 SHA-256 校验后的 tar 包直接蓝绿部署，远端主分支同步仍待网络恢复后完成。
